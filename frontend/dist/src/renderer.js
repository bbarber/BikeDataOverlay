...
let hrZones = {};
electron_1.ipcRenderer.on('metrics-update', (event, metrics) => {
    updateMetricsDisplay(metrics);
});
electron_1.ipcRenderer.on('connection-status-changed', (event, status) => {
...
async function fetchMetrics() {
    try {
        const metrics = await electron_1.ipcRenderer.invoke('get-current-metrics');
        updateMetricsDisplay(metrics);
    }
    catch (error) {
...
async function scanForDevices() {
    ...
    try {
        ...
        const result = await electron_1.ipcRenderer.invoke('scan-for-devices', 15);
        if (statusEl) {
...
async function loadDeviceList() {
    ...
    try {
        ...
        const result = await electron_1.ipcRenderer.invoke('list-devices');
        if (statusEl) {
            statusEl.textContent = result.message;
        }
        displayDevices(result.devices);
...
async function connectToDevice(deviceId) {
    ...
    try {
        ...
        const result = await electron_1.ipcRenderer.invoke('connect-to-device', deviceId);
        if (result.success) {
...
function updateHrInputs() {
    ...
    if (ageInput)
        ageInput.value = hrConfig.age.toString();
    if (restingHRInput)
        restingHRInput.value = hrConfig.restingHR.toString();
}
...
function updateHeartRateDisplay(heartRate) {
    ...
    if (heartRate > 0) {
        if (hrElement)
            hrElement.textContent = Math.round(heartRate).toString();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const disableMouseEvents = () => {
        electron_1.ipcRenderer.send('set-ignore-mouse-events', true);
    };
